...
- `images/*.svg`：浓缩、美式、拿铁、卡布奇诺和低因美式五张本地饮品图片，不依赖远程资源。
- `preview.html` / `preview.ts`：不依赖公共 registry 的独立交互预览，可调整参数、切换电源与指示灯状态、模拟用户接近、从页面底部的图片菜单选择咖啡，并开合水箱翻盖。
...
预览中的菜单只在电源开启且模拟用户进入正面接近范围后显示。页面底部只展示饮品图片和名称，不向饮用者展示水量、奶量、豆种或库存等内部细节。选择饮品仍会按照配方在内部扣减库存；容量、初始库存和配方均可通过 `operations.ts` 导出的纯函数与类型配置。
