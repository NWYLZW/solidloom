...
- `cyber-factory-warehouse-stacker-crane`：默认采用与三跨货架对齐的 3.5 米轨道、2.6 米紧凑单立柱和 0.7 米货叉行程，也可参数化扩展轨道、高度、载货台和货叉；输入稳定格口 ID 后可生成确定性的九步取货计划。
...
`planWarehouseRetrieval(slotId, rackParameters, craneParameters)` 接受形如 `warehouse-rack-slot-b03-l04` 的稳定格口 ID，校验货架范围、轨道横移行程和货叉伸出行程，然后生成预占、横移、升降、伸叉、挂接、收叉、下降、移动到左侧出库位和在载货台内释放九个确定性步骤。每一步都包含横移、升降和货叉伸出姿态，预览会按这些姿态演示完整取货过程。
